...
namespace Frost
{
	...
	namespace VulkanUtils
	{
		...
		static ShaderType ShaderTypeFromString(const std::string& type)
		{
			...
			if (type == "compute")						return ShaderType::Geometry;
		...
		: m_Filepath(filepath)
	{
		...
		auto shaderSources = VulkanUtils::PreProccesShaders(source);
	...
	void VulkanShader::CreateShaderModules()
	{
		...
		{
			auto& shaderModule = m_ShaderModules[VulkanUtils::ShaderTypeToStage(vulkanSPIRVSourceShader.first)];

			shaderModule = VulkanUtils::CreateShaderModule(vulkanSPIRVSourceShader.second);
	...
	void VulkanShader::CompileVulkanBinaries(std::unordered_map<ShaderType, std::string> shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			std::filesystem::path cachedPath = cacheDirectory / (shaderFilePath.filename().string() + VulkanUtils::ShaderStageCachedFileExtension(stage));
			...
			else
			{
				shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source,
													   VulkanUtils::ShaderStageToShaderC(stage), m_Filepath.c_str(), options);

				if (module.GetCompilationStatus() != shaderc_compilation_status_success)
				{
					FROST_CORE_ERROR("Error in {0} shader:\n {1}", VulkanUtils::ShaderTypeToString(stage), module.GetErrorMessage());
					FROST_ASSERT(false, "");
				}
	...
	void VulkanShader::CreateVulkanDescriptorSetLayout()
	{
		...
		for (auto& descriptorSetNumber : reflectedData.GetDescriptorSetsCount())
		{
			...
			for (auto& buffer : reflectedData.GetBuffersData())
			{
				...
				LayoutBinding.descriptorType = VulkanUtils::BufferTypeToVulkan(buffer.Type);
				LayoutBinding.stageFlags = VulkanUtils::GetShaderStagesFlagsFromShaderTypes(buffer.ShaderStage);
			}
			...
			for (auto& texture : reflectedData.GetTextureData())
			{
				...
				LayoutBinding.descriptorType = VulkanUtils::TextureTypeToVulkan(texture.Type);
				LayoutBinding.stageFlags = VulkanUtils::GetShaderStagesFlagsFromShaderTypes(texture.ShaderStage);
			}
			...
			for (auto& accelerationStructure : reflectedData.GetAccelerationStructureData())
			{
				...
				LayoutBinding.stageFlags = VulkanUtils::GetShaderStagesFlagsFromShaderTypes(accelerationStructure.ShaderStage);
			}
	...
	ShaderReflectionData VulkanShader::GetShaderReflectionData()
	{
		ShaderReflectionData reflectedData;
		reflectedData.SetReflectionData(m_VulkanSPIRV);
...
#define NULL_HANDLE 0xcdcdcdcdcdcdcdcd
		if (m_DescriptorPool == (VkDescriptorPool)NULL_HANDLE) return;
	...
	{
		return VulkanUtils::ShaderTypeToStage(type);
	}
}