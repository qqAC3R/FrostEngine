...
namespace ImGui
{
	...
	bool TreeNodeWithIcon(Frost::Ref<Frost::Texture2D> icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint /*= IM_COL32_WHITE*/)
	{
		...
		constexpr ImU32 backgroundDarkColor = IM_COL32(26, 26, 26, 255);
		const ImU32 arrow_col = selected ? backgroundDarkColor : mutedColor;
		...
		if (display_frame)
		{
			...
			if (icon)
			{
				...
				ImGui::SetCursorPosX(ImGui::GetCursorPosX() + arrowWidth + 5.0f);
				ImGui::SetCursorPosY(ImGui::GetCursorPosY() - frame_height + pad - 5.0f);
				Frost::ImGuiLayer* imguiLayer = Frost::Application::Get().GetImGuiLayer();
				ImTextureID textureID = imguiLayer->GetImGuiTextureID(icon->GetImage2D());
				ImGui::Image(textureID, { frame_height - pad * 0.5f, frame_height - pad * 0.5f }, ImVec2(0, 0), ImVec2(1, 1), iconTint /*selected ? colourDark : tintFloat*/);
