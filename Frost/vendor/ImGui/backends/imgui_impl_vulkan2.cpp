...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
    ...
    io.BackendFlags |= ImGuiBackendFlags_RendererHasViewports;  // We can honor the ImDrawCmd::VtxOffset field, allowing for large meshes.
...
void ImGui_ImplVulkan_Resize(uint32_t width, uint32_t height)
{
    auto bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;

    ImGui_ImplVulkanH_CreateOrResizeWindow(v->Instance, v->PhysicalDevice, v->Device, bd, v->QueueFamily, v->Allocator, (int)width, (int)height, v->MinImageCount);
}
