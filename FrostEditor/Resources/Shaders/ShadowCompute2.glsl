...
layout(binding = 3, rgba8) uniform restrict writeonly image2D u_ShadowTextureOutput;
...
float AverageBlockDepth(vec4 projCoords, uint cascadeIndex, float w_light, out float occludedCount, out float z_occSum);
...
const uint DPCF_SHADOW_TAP_COUNT                = 16u;
...
void blockerSearchAndFilter(vec4 shadowPosition, uint cascadeIndex, const mat2 randomRotation, vec2 filterRadii, float shadowBulbRadius, const uint tapCount, out float occludedCount, out float z_occSum)
{
    ...
    for (uint i = 0; i < tapCount; i++)
	{
        ...
        float z_bias = 0.002;
		//if(cascadeIndex == 1)
		//	z_bias = 0.0015 * shadowBulbRadius;
		//if(cascadeIndex == 2)
		//	z_bias = 0.0018 * shadowBulbRadius;
		//if(cascadeIndex == 3)
		//	z_bias = 0.003 * shadowBulbRadius;
 ...
 */
float ShadowSample_DPCF(vec4 shadowPosition, uint cascadeIndex, float zLight)
{
    ...
    vec2 dz_duv = vec2(0.05);


	float penumbraScale = 1.0;
	float shadowBulbRadius = max(zLight, 0.5);
	float penumbra = penumbraScale * shadowBulbRadius;
	...
	blockerSearchAndFilter(
		shadowPosition, cascadeIndex, rot, texelSize * vec2(penumbra), penumbra, DPCF_SHADOW_TAP_COUNT, occludedCount, z_occSum
	);
	//AverageBlockDepth(shadowPosition, cascadeIndex, zLight, occludedCount, z_occSum);
	...
	float blocker = z_occSum / occludedCount;
	float receiverDepth = shadowPosition.z;
	float penumbraRatio = (receiverDepth - blocker) / (blocker);
	
	//const float NEAR = 0.002;
	//float uvRadius = penumbraRatio * penumbra * NEAR / blocker;
	...
	//float penumbraRatioScale = 1.0;
	//penumbraRatio *= penumbraRatioScale;
	//
	...
	float percentageOccluded = PCFDirectionalLight(shadowPosition, cascadeIndex, penumbraRatio * zLight * NEAR / receiverDepth, blocker);
	//float percentageOccluded = SampleShadowMap_Gather((shadowCoords.xy) + offset, cascadeIndex);
...
float PCFDirectionalLight(vec4 shadowCoords, uint cascadeIndex, float uvRadius, float blockerDistance)
{
	...
	float sum = 0.0;
	for (int i = 0; i < requiredPCFSamples; i++)
	{
		vec2 offset;
		//if(cascadeIndex <= 2)
...
float PCSS_SampleShadowTexture(vec4 shadowCoords, uint cascadeIndex, float lightSize, float customDepth)
{
	...
	mat2 rot = RandomRotation(vec2(gl_GlobalInvocationID.xy));
	blockerSearchAndFilter(
		shadowCoords, cascadeIndex, rot, vec2(1.0), lightSize, 12u, occludedCount, occlusionSum
	);
...
void main()
{
	...
	*/
	...
	shadowFactor = u_DirLightData.UsePCSS == 1 ? ShadowSample_DPCF(shadowCoord, cascadeIndex, lightSize) : HardShadows_SampleShadowTexture(shadowCoord, cascadeIndex);
