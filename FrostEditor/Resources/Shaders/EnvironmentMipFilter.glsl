...
layout(binding = 0, rgba16f) restrict writeonly uniform imageCube o_PrefilteredMap;
...
{
    vec2 st = gl_GlobalInvocationID.xy / vec2(imageSize(o_PrefilteredMap));
...
void main(void)
{
	...
	ivec2 outputSize = imageSize(o_PrefilteredMap);
	...
	vec2 inputSize = vec2(textureSize(u_RadianceMap, 0));
	...
	for(uint i = 0; i < NumSamples; i++)
	{
		...
		if(cosLi > 0.0) {
			...
			color  += textureLod(u_RadianceMap, Li, mipLevel).rgb * cosLi;
			weight += cosLi;
		}
	}
	color /= weight;

	imageStore(o_PrefilteredMap, ivec3(gl_GlobalInvocationID), vec4(color, 1.0));
}
